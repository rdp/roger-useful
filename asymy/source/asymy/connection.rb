...
require 'stringio'
require 'fiber' # for Fiber.current
...
module Asymy
	...
	# same as exec, except it also returns the args, as well as letting them be processed within the block
  	def exec_and_fiber_yield query
    		fiber = Fiber.current
    		incoming = nil
    		self.exec(query) {|*args| yield(*args); incoming = *args; fiber.resume }
    		Fiber.yield # continues when fiber.resume is called, above
    		return incoming
  	end
	...
	attr_accessor :em_connection
        ...
        def reco
            EventMachine::connect(@target, @port, Session) {|c| c.bp = self; self.em_connection = c} # might block for DNS
        end
